#!/usr/bin/python3.6
import argparse
parser = argparse.ArgumentParser(description='Passing arguments for processing tweets for a given date range')
parser.add_argument('-fromDate', action='store', dest='fromDate', default='', help='Date format YYYY-MM-DD . From date to search tweets from this date')
parser.add_argument('-toDate', action='store', dest='toDate', default='', help='Date Format YYYY-MM-DD . To date to search tweets till this date')
...
def main():
	'''
	This script is for downloading data from tweeter for search of hashtag #FlixBus from last week tweets.
	'''
	try:
